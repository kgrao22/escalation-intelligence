 ...
 */
export const RECURRENCE_ADJUDICATION_PROMPT_VERSION = "v2";

/*
 * v2 reworded the shared-subject trap list and two examples off the real
 * product vocabulary. That is instructional content rather than illustration,
 * so the version moved and prior v1 adjudications will be re-run rather than
 * reused against wording they were never judged under.
 *
 * The shared-subject trap list below ("payments, notifications, dashboards…")
 * is deliberately generic. Naming your own highest-frequency domain nouns there
 * makes the instruction bite harder, so tuning it to your product is worthwhile
 * — but it changes the prompt, so bump RECURRENCE_ADJUDICATION_PROMPT_VERSION
 * when you do, or stale adjudications will be reused against the new wording.
 */
...
Example: "Invoice tax excludes partner/platform fees" and "Invoice total omits tax for specific fee components" — same underlying calculation defect.
...
Example: "Payment link expires before the due date" and "Payment link fails because the fee calculation is incorrect" — same workflow, different defects.

different
No meaningful recurring engineering relationship beyond broad product or domain similarity.

BE CONSERVATIVE. Do NOT label two escalations same_underlying_issue merely because they both mention the same product, payments, notifications, dashboards, amendments, record creation, email, or APIs. Sharing a subject area is not sharing a defect. The bar for same_underlying_issue is that fixing the underlying problem once would plausibly resolve both.
...
Example: "payment link fails due to a fee mismatch" and "payment link fails due to an upstream API outage" are related_problem_family, not same_underlying_issue — identical symptom, unrelated causes.
...
- Example: "Incorrect tax calculation on invoice fee components".
